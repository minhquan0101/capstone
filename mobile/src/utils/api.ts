...
export async function createBookingWithSeats(
  eventId: string,
  seats: Array<{ seatId: string; ticketTypeId: string }>,
  paymentMethod: "momo" | "credit_card" | "bank_transfer" = "momo"
): Promise<any> {
  const token = await AsyncStorage.getItem("token");
  if (!token) {
    throw new Error("Bạn cần đăng nhập để đặt vé");
...
export async function getSeatStatus(eventId: string): Promise<{
  soldSeatIds: string[];
  heldSeatIds: string[];
}> {
  const res = await fetch(`${API_BASE}/events/${eventId}/seats/status`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.message || "Không tải được trạng thái ghế");
  }

  return {
    soldSeatIds: Array.isArray(data.soldSeatIds) ? data.soldSeatIds : [],
    heldSeatIds: Array.isArray(data.heldSeatIds) ? data.heldSeatIds : [],
  };
}
